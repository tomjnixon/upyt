    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                0,
                4,
            ),  # No overhead so will use insert and equal
...
class TestDataToUpdateCommands:
    ...
    def test_equal_blocking(self) -> None:
        assert list(
            data_to_update_commands(b"12345678910", b"12345678910", block_size=20)
        ) == [
            ("w(r(11))", 11),
        ]
        assert list(
            data_to_update_commands(b"12345678910", b"12345678910", block_size=11)
        ) == [
            ("w(r(11))", 11),
        ]
        assert list(
            data_to_update_commands(b"12345678910", b"12345678910", block_size=8)
        ) == [
            ("w(r(8))", 8),
            ("w(r(3))", 3),
        ]
    ...
    def test_insertion_blocking(self) -> None:
        assert list(
            data_to_update_commands(b"12345678910", b"XXX12345678910", block_size=2)
        ) == [
            ("w(b'XX')", 2),
            ("w(b'X')", 1),
            ("w(r(2))", 2),
    ...
    def test_seeking_after_replacement(self) -> None:
        ...
        # to be worth it!
        assert list(
            data_to_update_commands(b"1234567891011121314", b"XX34567891011121314")
        ) == [
            ("w(b'XX')", 2),
            ("s(2)", 0),
            ("w(r(17))", 17),
...
class TestPrimitives:
    ...
    def test_update_file_many_edits(self, ser) -> None:
        with upy_filesystem(ser) as fs:
            ...
            new_content = b"".join(
                bytes([c]) if (i % 2) else b"\00" for i, c in enumerate(old_content)
            )
            fs.write_file(f"{tmpdir}/foo", old_content)
            fs.update_file(f"{tmpdir}/foo", old_content, new_content)
            assert fs.read_file(f"{tmpdir}/foo") == new_content
